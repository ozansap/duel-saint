  ...
  filtered?.forEach(async (c) => {
    ...
    if (subcommands.length !== 0) {
      for (const s of subcommands) {
        description += `</${c.name} ${s.name}:${c.id}>⠀•⠀${c.defaultMemberPermissions?.toArray()[0]}⠀•⠀${s.description}\n`;
      }
    } else {
      description += `</${c.name}:${c.id}>⠀•⠀${c.defaultMemberPermissions?.toArray()[0]}⠀•⠀${c.description}\n`;
    }
  });
